...
export class CacheService {
  ...
  async getItem(key : string) {
    const {created, data} = await this.cacheRepository.getOrCreate(key)
    return {created : created, data : new CacheResponse(data.id, data.key, data.value, data.expirationDate)}
  }

  async getAllKeys() : Promise<string[]> {
    const data = await this.cacheRepository.getAllData()
    return data.map(cache =>  cache.key)
  }

  async addOrUpdate(key : string, value : string){
    const {created, data} = await this.cacheRepository.addOrUpdate(key, value)
    return {created : created, data : new CacheResponse(data.id, data.key, data.value, data.expirationDate)}
  }
